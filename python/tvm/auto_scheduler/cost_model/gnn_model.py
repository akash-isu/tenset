...
from torch_geometric.nn import GCNConv, GatedGraphConv, TransformerConv
...
class ASTGNNModule(torch.nn.Module):
  def __init__(self, vocab_size):
    super().__init__()
    self.embeddings = torch.nn.Embedding(vocab_size + 1, 16)
    self.conv1 = TransformerConv(256, 64, add_self_loops=True, aggr='max')
    # self.conv2 = GatedGraphConv(64, 32, add_self_loops=True)
    # self.conv3 = GatedGraphConv(32, 12, add_self_loops=True)
    # self.conv4 = GatedGraphConv(12, 6, add_self_loops=True)
  ...
  def forward(self, data):
    ...
    g = self.embeddings(x)
    g = g.view(*g.shape[:1], -1)
    # g = torch.nn.functional.softmax(g)
    g = self.conv1(g, edge_index)
    # g = torch.nn.functional.softmax(g)
...
def make_net(params):
    ...
    elif params["type"] == "BaseGNN":
        vocab_file = open("tvm_bert/tvm_bert_it-vocab.txt", 'r')
        vocab_size = len(vocab_file.readlines())
        vocab_file.close()
        return ASTGNNModule(vocab_size)
    else:
...
class GNNModelInternal:
    ...
    def predict(self, dataset):
        if self.few_shot_learning in ["base_only", "fine_tune_mix_task", "fine_tune_per_task", "MAML"]:
            ...
            # print(loader)
            # print(dataset)
            return self._predict_a_dataset(self.base_model, loader)
    ...
    def _fit_a_model(self, train_set, valid_set=None, valid_train_set=None, n_epoch=None):
        ...
        if valid_set:
            # for task in valid_set.tasks():
            #     self.register_new_task(task) //***********might need later
        ...
        for epoch in range(n_epoch):
            ...
            for idx, batch in enumerate(train_loader):
                ...
                # batch.x = batch.x.type(torch.cuda.LongTensor)
                batch.y = batch.y.reshape(-1,1)
                # batch.y.to(self.device)
                y_pred = net(batch.to(self.device))
                # print("y_pred", y_pred, y_pred.shape, batch.y.shape)
                # print("label", batch['y'])
                loss = self.loss_func(y_pred, batch['y'].type(torch.FloatTensor).to(self.device))
                loss.backward()
            ...
            if epoch % self.print_per_epoches == 0 or epoch == n_epoch - 1:
                ...
                print("Epoch: %d\tBatch: %d\t%s\tTrain Speed: %.0f" % (
                    epoch, idx, loss_msg, len(train_loader) / train_time,))
    ...
    def _validate(self, model, valid_loader):
        ...
        for batch in valid_loader:
            ...
            valid_losses.append(self.loss_func(preds, batch['y'].type(torch.FloatTensor).to(self.device)).item())

        return np.mean(valid_losses)

    def _predict_a_dataset(self, model, loader):
        ret = []
        labels = []
        for batch in loader:
            batch.batch.to(self.device)
            ...
            # batch.y.to(self.device)
            preds = model(batch.to(self.device))
            ret.extend(preds.detach().cpu().numpy())
        ...
        return np.array(ret), np.array(labels)
